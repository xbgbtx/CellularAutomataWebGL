...
function process_audio ()
{
    ...
    let r_size = 32;
    let r_data = reduce_audio_data ( r_size, 0, audio_data.length  );
...
function render_reduced_data ( r_data )
{
    ...
    {
        let n = i / r_data.length;
        let a = Math.PI * 2 * n;
        let h = dist * n * r_data[i];

        let x = origin.x + ( h * Math.cos ( a ) );
        let y = origin.y + ( h * Math.sin ( a ) );

        let rad = dist * (1-n) * r_data[i];

        ca_system.renderer.draw ( x, y, pen_col, rad );
    }
}

function old_render_reduced_data ( r_data )
{
