...
function show_automata_menu ()
{
    ...
    let show_automaton_params = ( a ) => 
    {
        ...
        let els = create_automata_params_elements ( params );
        ...
        rand_button.onclick = () => console.log ( "Randomize parms..." );
    ...
    a_select.onchange = () => show_automaton_params ( a_select.value );
...
function create_automata_params_elements ( params )
{
    ...
    for ( const [ p_name, p_value ] of params ) 
    {
        ...
        {
            let p_fs = document.createElement ( "fieldset" );
            let p_leg = document.createElement ( "legend" );
            p_leg.textContent = p_name;
            p_fs.appendChild ( p_leg );

            for ( const val of p_value.domain () ) 
            {
                let label = document.createElement ( "label" );
                label.textContent = `${val}:`;
                p_fs.appendChild ( label );

                let checkbox = document.createElement ( "input" );
                checkbox.type = "checkbox";
                checkbox.id = `auto_param_${p_name}_${val}`;
                p_fs.appendChild ( checkbox );
