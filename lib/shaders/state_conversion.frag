...
{
   int cInt [ 3 ] = int [ 3 ] ( int ( c.r * 255.0 ), 
                                int ( c.r * 255.0 ), 
                                int ( c.r * 255.0 ) );
...
vec4 sVec_to_color ( vec4 sVec )
{
   ...
   int idx = sInt * 3;
   return vec4 ( float ( state_colors [ idx ] ) / 255,
                 float ( state_colors [ idx + 1 ] ) / 255,
                 float ( state_colors [ idx + 2 ] ) / 255,
                 1.0 );
}
